...
class ActorNet(nn.Module):
    def __init__(self, state_dim=8, action_dim=2, hidden_dim=(400, 300)):
        ...
        self.fc1 = nn.Linear(state_dim, hidden_dim_1)#(8,400)
        self.fc2 = nn.Linear(hidden_dim_1, hidden_dim_2)#(400,300)
        self.fc3 = nn.Linear(hidden_dim_2, action_dim)#(300,2)
...
class CriticNet(nn.Module):
    def __init__(self, state_dim=8, action_dim=2, hidden_dim=(400, 300)):
        ...
        self.critic_head = nn.Sequential(
            nn.Linear(state_dim + action_dim, h1), #(10,400)
            nn.ReLU(),
        )
        self.critic = nn.Sequential(
            nn.Linear(h1, h2),
            nn.ReLU(),
            nn.Linear(h2, action_dim),#(300,2)
        )
...
def test(args, env, agent, writer):
    ...
    for n_episode, seed in enumerate(seeds):
        ...
        while True:
            env.render()
            action = agent.select_action(state) # selection
...
def main():
    ...
    parser.add_argument('--logdir', default='log/ddpg2')
    # train
