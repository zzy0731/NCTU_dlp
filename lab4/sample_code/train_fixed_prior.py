...
def train(x, cond, modules, optimizer, kl_anneal, args, device):
    ...
    for i in range(1, args.n_past + args.n_future):
        ...
        if (use_teacher_forcing):
            h, _ = modules['encoder'](x_pred_seq[i-1])
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        for _ in range(args.epoch_size):
            ...
            loss, mse, kld = train(seq, cond, modules, optimizer, kl_anneal, args,device=device)
            epoch_loss += loss
            epoch_mse += mse
            epoch_kld += kld
        ...
        if epoch % 5 == 0:
            ...
            for _ in range(len(validate_data) // args.batch_size):
                try:
                    train_seq, train_cond = next(train_iterator)
                except StopIteration:
                    train_iterator = iter(train_loader)
                    validate_seq, validate_cond = next(validate_iterator)

                pred_seq2 = pred(train_seq, train_cond, modules, args, device)
                train_seq = train_seq.permute(1,0,2,3,4)
                _, _, psnr2 = finn_eval_seq(train_seq[args.n_past:], pred_seq2[args.n_past:])
                psnr_list2.append(psnr2)
            ave_psnr2 = np.mean(np.concatenate(psnr2))
            with open('./{}/train_record.txt'.format(args.log_dir), 'a') as train_record:
            ...
            if ave_psnr > best_val_psnr:
                ...
                plot_pred(validate_seq, validate_cond, modules, epoch, args,lasttext='best_validate')
                ...
                plot_pred(train_seq, train_cond, modules, epoch, args,lasttext='best_train')
        ...
        if epoch % 20 == 0 or epoch == (start_epoch + niter):
            ...
            validate_seq = validate_seq.to(device)
            validate_cond = validate_cond.to(device)
            plot_pred(validate_seq, validate_cond, modules, epoch, args,lasttext='validate')
            ...
            train_seq = train_seq.to(device)
            train_cond = train_cond.to(device)
            plot_pred(train_seq, train_cond, modules, epoch, args,lasttext='train')
